...
@Component({
  selector: 'vef-weekly-expenses',
  templateUrl: './weekly-expenses.component.html',
  styleUrls: ['./weekly-expenses.component.scss'],
})
export class WeeklyExpensesComponent implements OnInit {
  sales: any[] = [];
  size = 10;
  page = 1;
  totalItems = 0;
  ...
  getOcassions(size: number, page: number) {
    ...
    this.service
      .getPaginated({size, page}, '/Sales/weekly/paged')
      .subscribe({
        next: (res: any) => {
